    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 rounded-xl bg-white p-6 shadow-sm border border-gray-100">
            ...
            <dl class="grid grid-cols-2 gap-4">
                ...
                <div><dt class="text-xs font-medium text-gray-500 uppercase">Reported By</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->reportedBy?->name }}</dd></div>
                <div class="col-span-2"><dt class="text-xs font-medium text-gray-500 uppercase">Fault Description</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->fault_description }}</dd></div>
                <div><dt class="text-xs font-medium text-gray-500 uppercase">Vendor</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->vendor_name ?? '—' }}</dd></div>
                <div><dt class="text-xs font-medium text-gray-500 uppercase">Repair Cost</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->repair_cost ? number_format($repairRecord->repair_cost, 2) : '—' }}</dd></div>
                <div><dt class="text-xs font-medium text-gray-500 uppercase">Sent Date</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->sent_date ? \Carbon\Carbon::parse($repairRecord->sent_date)->format('M d, Y') : '—' }}</dd></div>
                <div><dt class="text-xs font-medium text-gray-500 uppercase">Return Date</dt><dd class="mt-1 text-sm text-gray-900">{{ $repairRecord->return_date ? \Carbon\Carbon::parse($repairRecord->return_date)->format('M d, Y') : '—' }}</dd></div>
        ...
        <div class="space-y-6">
            <div class="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
                ...
                <span class="inline-flex rounded-full px-3 py-1 text-sm font-medium {{ $rColors[$repairRecord->status] ?? 'bg-gray-100 text-gray-600' }}">{{ ucfirst(str_replace('_', ' ', $repairRecord->status)) }}</span>
